...
class SimulationDirectory:
    ...
    def write_files(self, simname='SIMULATION', user_input=True, sim_config=None):
        ...
        def write_svfsixml_input_params(user_input=user_input, sim_config=sim_config):
            ...
            inflow_boundary_condition = (
                "neumann"
                if sim_config is None
                else str(sim_config.get("inflow_boundary_condition", "neumann")).lower()
            )
            ...
            if self.svzerod_3Dcoupling is not None:
                ...
                if inflow_boundary_condition == "dirichlet":
                    if self.zerod_config is None:
                        raise ValueError(
                            "dirichlet inflow_boundary_condition requires zerod_config "
                            "to generate inflow.flow"
                        )
                    inflow_n_tsteps = 2000
                    if inflow_period is not None and dt > 0.0:
                        inflow_n_tsteps = max(int(round(inflow_period / dt)), 2)
                    inflow_file_path = os.path.join(self.path, "inflow.flow")
                    self.zerod_config.generate_inflow_file(
                        self.path,
                        period=inflow_period,
                        n_tsteps=inflow_n_tsteps,
                    )
            self.svFSIxml.write(self.mesh_complete,
                                ...
                                inflow_boundary_condition=inflow_boundary_condition,
                                inflow_file_path=inflow_file_path,
    ...
    def plot_mpa(self, clinical_targets=None, plot_pf_loop=True, window="last", full_cycle=True, cycle_duration=1.0, last_cycle_only=None, last_full_cycle=None):
        '''
