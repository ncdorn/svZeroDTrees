...
class ConfigHandler():
    ...
    def  generate_threed_coupler(self, simdir, inflow_from_0d=True, mesh_complete=None, include_distal_vessel=False):
        ...
        if inflow_from_0d:
            ...
            inflow_bc_names = [
                bc_name for bc_name in list(threed_coupler.bcs.keys())
                if "inflow" in bc_name.lower()
            ]
            self.n_inflows = len(inflow_bc_names)
            for bc_name in inflow_bc_names:
    ...
    def generate_inflow_file(self, simdir, period=None, n_tsteps=None, header_terms=16):
        '''
        ...
        inflow_name = next(
            (bc_name for bc_name in self.bcs.keys() if "inflow" in bc_name.lower()),
            None,
        )
        if inflow_name is None:
            raise ValueError("cannot generate inflow.flow without an inflow boundary condition")

        inflow_bc = self.bcs[inflow_name]
        times = np.asarray(inflow_bc.values["t"], dtype=float)
        flows = np.asarray(inflow_bc.values["Q"], dtype=float)
        if times.size == 0 or flows.size == 0 or times.size != flows.size:
            raise ValueError("inflow boundary condition must define matching non-empty t and Q arrays")

        times = times - float(times.min())
        resolved_period = float(period) if period is not None else float(times.max())
        if resolved_period <= 0.0:
            raise ValueError(f"invalid inflow period for inflow.flow generation: {resolved_period}")

        resolved_n_tsteps = int(n_tsteps) if n_tsteps is not None else int(times.size)
        if resolved_n_tsteps < 2:
            raise ValueError("inflow.flow generation requires at least 2 time samples")

        if times[-1] <= 0.0:
            times = np.linspace(0.0, resolved_period, flows.size)

        if times[-1] < resolved_period:
            times = np.append(times, resolved_period)
            flows = np.append(flows, flows[0])

        sample_times = np.linspace(0.0, resolved_period, resolved_n_tsteps)
        sample_flows = np.interp(sample_times, times, flows)

        with open(os.path.join(simdir, 'inflow.flow'), 'w') as ff:
            ff.write(f"{resolved_n_tsteps} {int(header_terms)}\n")
