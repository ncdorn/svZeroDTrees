...
def test_noncoupled_impedance_config_with_tree_metadata_remains_solver_compatible(monkeypatch, fixtures_dir, tmp_path):
    config_path = fixtures_dir / "example_pa_config.json"
    ...
    monkeypatch.setattr(
        pysvzerod,
        "simulate",
        lambda _payload: pd.DataFrame({"time": [0.0], "pressure_in": [1.0]}),
    )
