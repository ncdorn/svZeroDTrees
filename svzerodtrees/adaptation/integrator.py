...
def run_adaptation(preop_pa, postop_pa, model, K_arr):
    ...
    geom_err        = rel_change(y_final, y_initial)  # overall geometry change
    ...
    # hists = [0, 0, 0, 0]
    # hists[0] = r_h_histogram(y0, n_lpa)
    # hists[1] = r_h_histogram(y_third, n_lpa)
    # hists[2] = r_h_histogram(y_two_third, n_lpa)
    # hists[3] = r_h_histogram(y_final, n_lpa)

    hists = [plot_adaptation_histories(sol.y, n_lpa)]

    # Indices
    total_states = y_initial.size
    lpa_r_idx = np.arange(0, 2 * n_lpa, 2)     # even indices for LPA radii
    lpa_h_idx = np.arange(1, 2 * n_lpa, 2)     # odd indices for LPA thicknesses
    rpa_r_idx = np.arange(2 * n_lpa, total_states, 2)
    rpa_h_idx = np.arange(2 * n_lpa + 1, total_states, 2)

    # Compute mean relative changes
    mean_r_change_lpa = np.mean((y_final[lpa_r_idx] - y_initial[lpa_r_idx]) / y_initial[lpa_r_idx])
    mean_h_change_lpa = np.mean((y_final[lpa_h_idx] - y_initial[lpa_h_idx]) / y_initial[lpa_h_idx])
    mean_r_change_rpa = np.mean((y_final[rpa_r_idx] - y_initial[rpa_r_idx]) / y_initial[rpa_r_idx])
    mean_h_change_rpa = np.mean((y_final[rpa_h_idx] - y_initial[rpa_h_idx]) / y_initial[rpa_h_idx])
